defmodule MustacheFeatureTest do
  ...
  test "Triple Mustache Decimal Interpolation" do
    assert Mustache.render("\"{{{power}}} jiggawatts!\"", %{power: 1.21}) ==
             "\"1.21 jiggawatts!\""
  end
  ...
  test "Section Interpolation - Basic and Inverse" do
    assert Mustache.render(
             "\"{{person.name}}\" == \"{{#person_2}}{{name}}{{/person_2}}{{^person_2}}Joe{{/person_2}}\"",
             %{person: %{name: "Joe"}}
           ) == "\"Joe\" == \"Joe\""
  end
  ...
  test "Section Interpolation - list values" do
    assert Mustache.render(
             "\"{{#people}}{{name}} {{/people}}\"",
             %{people: [%{name: "Joe"}, %{name: "Jill"}]}
           ) == "\"Joe Jill \""
  end

  test "render/2 returns rendered template when key is string" do
    assert Mustache.render("Hello {{custom.candidate}}.", %{
             custom: %{"candidate" => "Palpatine"}
           }) == "Hello Palpatine."
  end
